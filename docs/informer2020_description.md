...
Этот документ описывает модель Informer, ее архитектуру, область применения, основные параметры (включая гиперпараметры, подходящие для тюнинга с помощью Optuna), а также структуру репозитория `Informer2020`.

---

## 1. Назначение модели и область применения

**Informer** — это архитектура для долгосрочного прогнозирования временных рядов на основе Transformer-подобной модели с эффективным вниманием (*ProbSparse Attention*). Основные цели:
...
Соответствующие размеры входов/выходов (`enc_in`, `dec_in`, `c_out`) задаются через словарь `data_parser` в `main_informer.py` в зависимости от выбранного датасета (`--data`).
...
3. **TemporalEmbedding / TimeFeatureEmbedding**
   - Если `embed != "timeF"`: используется `TemporalEmbedding`, где каждая компонент календарного признака (час, день недели, день месяца, месяц, при необходимости минуты) кодируется через фиктивный или обучаемый `Embedding`.
   - Если `embed == "timeF"`: используется `TimeFeatureEmbedding`, который преобразует набор непрерывных временных признаков (нормализованных в диапазоне [-0.5, 0.5]) линейным слоем в пространство размерности `d_model`.
...
- **EncoderStack**
  - Используется в варианте `InformerStack`, когда несколько энкодеров с разной глубиной объединяются, а выходы конкатенируются по временной оси.
...
- `train(setting)`:
  ...
  - По окончании загружает лучшее состояние модели (по валидационному loss) из `./checkpoints/<setting>/checkpoint.pth`.
...
Ниже перечислены параметры, которые наиболее часто имеют смысл оптимизировать с помощью Optuna (или любого другого HPO-инструмента). Конкретные диапазоны зависят от задач и ограничений по ресурсам.
...
3. Запуска эксперимента:
   - `_run_informer_and_get_metrics` формирует команду вызова `main_informer.py`, запускает её, измеряет время выполнения и (опционально) энергию GPU через ZeusMonitor.
...
1. **Подготовка окружения**
   - `make init` для сборки Docker-образа (по желанию).
   - Либо установка зависимостей через `pip install -r requirements.txt`.

2. **Загрузка данных**
   - `make dataset` для ETT/ECL/WTH (если используется стандартный набор).

3. **Запуск эксперимента**
   - Прямой вызов:
     ```bash
     python -u main_informer.py --model informer --data ETTh1 --features M --seq_len 96 --label_len 48 --pred_len 24
     ```
   - Или через один из скриптов в `scripts/`.

4. **Интеграция с Optuna**
   - Внешний скрипт генерирует набор гиперпараметров.
   - Запускает `informer_experiment_wrapper.py`:
     ```bash
     python -u informer_experiment_wrapper.py        --data-path /path/to/ETTh1.csv        --metrics-path /path/to/metrics.json        --epochs 10        -- --features M --seq_len 96 --label_len 48 --pred_len 24
     ```
   - Читает `metrics.json` и использует выбранную метрику (например, `mse`) в качестве целевой функции.

5. **Анализ результатов**
   - `results/<setting>/metrics.npy`: численные значения метрик.
   - `results/<setting>/pred.npy` и `true.npy`: можно использовать для построения графиков.
   - CSV с выпрямленными значениями предсказаний/истин (`*_valid_predictions.csv`, если запускался wrapper).
